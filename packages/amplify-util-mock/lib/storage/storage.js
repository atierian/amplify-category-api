...
class StorageTest {
    ...
    async trigger(context) {
        ...
        this.storageSimulator.getServer.on('event', async (eventObj) => {
            ...
            const config = await (0, load_lambda_config_1.loadLambdaConfig)(context, triggerName);
            const invoker = await (0, amplify_category_function_1.getInvoker)(context, { handler: config.handler, resourceName: triggerName, envVars: config.environment });
            try {
